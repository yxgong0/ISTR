...
#         assert _input.shape == target.shape
#
#         _input = self.sort_tensor(_input)
#         target = self.sort_tensor(target)
#
#
#         _input[0, 0, i] = True in torch.eq(target, _input[0, 0, i]) for i in range(_input.shape[-1])
#         # loss = self.criterion(_input, target)
#         numel = _input.shape[-1]
#         torch.topk()
#
#         return loss
#
